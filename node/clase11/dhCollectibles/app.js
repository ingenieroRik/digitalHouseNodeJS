...
let importa = require ("./collectibles.js");

//creamos 3 variables con las marcas
let marcaStarWars = importa ("Star Wars");
let marcaHotToys = importa ("Hot Toys");
let marcaBandia = importa ("Bandia");

//creamos un array que contiene a los 3 anteriores con spread operator
let unifiedCollectibles = [...marcaHotToys, ...marcaBandia, ...marcaStarWars];

     console.table(unifiedCollectibles);

// creamos el objeto
let collectibles = {
    figuras : unifiedCollectibles,
    
    listFiguras : function () {this.figuras.forEach(function(elemento){
            console.log("*****************");
            console.log("Marca: " + elemento.marca);
            console.log("Marca: " + elemento.nombre);
            console.log("Marca: " + elemento.precio);
            console.log("Marca: " + elemento.stock);
        });
    
    },

    //funcion para recibir por parametro la marca y devolver todas las figus de esa marca
    figuresByBrand : function(marca){
        let marcasfiguras = this.figuras.filter(function(figura){
            // que el id del objeto sea igual que el id que le pasamos a la funcion
            return figura.marca == marca ;
        });
        return marcasfiguras;
    }
}
